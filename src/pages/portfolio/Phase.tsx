...
export default function PhasePage() {
  ...
  const queryString = `?projectId=${projectId || ''}&phaseId=${phaseId || ''}&businessId=${businessId}`;
      ...
      <header className="flex items-center justify-between mb-6">
        <div className="flex gap-2">
          <div className="flex items-center gap-2">
            <Link to={`/dashboard/portfolio/saas/project/${projectId || ''}`}>
          ...
          <Breadcrumbs
            items={[
              ...
              {
                label: phaseLabel,
                to: `/dashboard/portfolio/saas/project/${projectId || ''}/phase/${phaseId || ''}`,
              },
            ]}
          />
        </div>
                    ...
                    <Draggable key={cat.id} draggableId={cat.id} index={index}>
                      {(provided, snapshot) => (
                        <div
                          ...
                          className={`transition-all w-64 max-w-xs ${snapshot.isDragging ? "z-50" : ""
                            }`}
                        >
                          <Link to={`${cat.to}${queryString}`} className="contents relative group">
                            ...
                            <motion.div
                              className={`flex flex-col items-center gap-3 w-full cursor-pointer p-6 rounded-[2.5rem] hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-all font-bold ${snapshot.isDragging
