...
const Name = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("bizConcept_name");
    if (saved) setValue(saved);
  }, []);

  const handleContinue = () => {
    localStorage.setItem("bizConcept_name", value);
    navigate("/dashboard/portfolio/questions/image");
  };
  ...
  return (
    <div className="flex flex-col h-full bg-[#f0f0eb] dark:bg-slate-950 p-6 sm:p-12 font-sans text-gray-900 dark:text-gray-100 justify-center items-center relative min-h-screen transition-colors duration-300">
      ...
      <Link
        to="/dashboard/portfolio/questions/image"
        onClick={() => localStorage.setItem("bizConcept_name", value)}
      ...
      <div className="max-w-3xl w-full flex flex-col gap-8">
        ...
        <div className="flex flex-col gap-4">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-lg leading-relaxed font-['Inter']">
            Enter the name of your business or startup.
          </p>
        </div>
        ...
        <div className="w-full">
          <input
            ...
            className="w-full p-6 rounded-3xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 dark:focus:border-blue-500 transition-all text-lg font-['Inter'] text-gray-900 dark:text-gray-200"
          />
        </div>
      </div>
